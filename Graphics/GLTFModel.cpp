...
public:
	...
	GLTFModelImpl(const char* file)
	{
		...
		GLTFModelImpl::file = file;
		pdata = get_data(JSON);
		traverse_node(JSON, 0, graphics::mat4(1.0));
		delete pdata;
	}
	...
	void traverse_node(json& JSON, unsigned int nextNode, graphics::mat4 matrix)
	{
	...
	void handle_vector_deletion(std::vector<Type*>* pvector)
	{
		if (pvector)
		{
...
{
	pimpl->draw(shader, camera, translation, rotation, scale);
}

bool GLTFModel::has_failed()
{
	return pimpl->load_failure;
}